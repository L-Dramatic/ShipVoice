...
实现上，baseline 模式对应 pipeline 的 response_mode=baseline：等待 LLM 输出完整文本后，再调用 TTS 合成完整音频并一次性返回。该基线可完成基本问答，但存在以下局限：（1）ASR 误听直接传递至 LLM，导致检索与回答偏移；（2）LLM 可能对越界或危险请求继续生成；（3）回答缺乏来源依据，难以审计；（4）首段可播放延迟较高，用户等待体验不佳。
...
| ASR 术语鲁棒性 | 在线转写 | CER/WER 1.58%，术语召回 85.71% |
