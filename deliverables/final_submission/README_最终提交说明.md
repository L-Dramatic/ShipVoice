...
| 改进点与局限 | 项目报告第 9 章、第 12 章 | 改进点包括安全门控、RAG 证据引用、术语后处理、多轮上下文、流式首句优先播放、浏览器首播计时、后台审计、真实 provider 接入；局限包括固定音频集仍非生产压测、等待体验为代理评分、SQLite 尚非生产数据库。 |
...
第三，系统有可量化评测证据。当前安全门控评测 56 条样本，决策准确率 100%，false allow 为 0；ASR 清单评测 50 条录音，后处理后术语召回 100%；多轮问答评测 6 组对话 18 轮，follow-up grounding accuracy 为 100%；引用质量评测 title hit@3 和 ID hit@3 均为 100%；真实语音链路在远程 GPU 环境完成 300 次 real-only 重复实验，gate-allowed 配对样本中流式改进平均节省 4147.39 ms，浏览器端 20/20 首播成功。
