...
| 模块版本与配置 | 项目报告第 5 章、`configs/pipeline.json`、`configs/runtime.*.env` | 说明 mock、real、full-real 三类运行模式，以及 ASR、LLM、TTS、RAG 的 provider 可替换设计。 |
| 基线 vs 改进指标表 | 项目报告第 8 章、`results/*.json` | 报告中对比串行基线、课程稳定演示链路、真实模型链路和安全增强链路，并解释当前瓶颈。 |
...
8. 如果使用真实 provider 演示，答辩前先启动远程服务；如果现场网络或 GPU 不稳定，切回 mock 模式演示稳定链路，同时展示真实链路实验结果作为证据。
