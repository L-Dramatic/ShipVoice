...
本文档用于最终打包提交和答辩前检查。按照《信息安全基础》期末项目要求，本组选题为 A2：级联式造船语音问答系统的复现与改进。课程要求提交项目源代码、说明文档以及答辩 PPT，并在说明文档中给出系统架构图、模块版本与配置、基线与改进方案的延迟和客观指标对比表、可复现实验步骤、改进点与局限。ShipVoice 的最终提交材料按这些要求组织，同时额外补充了安全门控、RAG 证据引用、真实语音链路验证、管理后台和运行审计，说明这不是单页网页演示，而是一套可运行、可测量、可复盘的船厂安全语音问答系统。
...
| 基线 vs 改进指标表 | 项目报告第 8 章、`results/server_real_repeated_20260623/summary.json`、`results/server_real_batch_comparison_20260623.json` | 报告中对比串行级联基线和流式低延迟改进，给出首段可播放延迟、配对节省和浏览器首播指标。 |
| 主观等待体验量化 | 项目报告第 8.6 节、`results/waiting_experience_20260623/` | 使用真实延迟日志生成自动化代理评分，不伪造真人问卷；可复现实验脚本为 `scripts/evaluate_waiting_experience.py`。 |
| 固定音频指令集 | `docs/FIXED_AUDIO_COMMAND_SET_20260623.md`、`data/audio/audio_manifest_a2_eval.csv` | 50 条录音按基础问答、专有名词、噪声/应急、安全边界分层，逐条列出评测重点。 |
| 可复现实验步骤 | `ShipVoice_可复现实验与运行手册.md` | 给出本地启动、后台登录、健康检查、单元测试、评测脚本、真实 provider 切换、远程 GPU 服务验证、音频集分层和等待体验评分等步骤。 |
| 改进点与局限 | 项目报告第 9 章、第 12 章 | 改进点包括安全门控、RAG 证据引用、术语后处理、多轮上下文、安全闭合流式首段优先播放、输出片段 guard、provider HTTP 连接复用、浏览器首播计时、后台审计、真实 provider 接入；局限包括固定音频集仍非生产压测、等待体验为代理评分、SQLite 尚非生产数据库。 |
| 项目源代码 | `source/` 目录 | 包含前端、后端、模型 provider 抽象、脚本、测试、配置、远程部署脚本。 |
| 说明文档 | `report/` 与 `manuals/` | 项目报告面向技术说明，运行手册面向复现，提交清单面向答辩前检查。 |
| 答辩 PPT | `slides/` | 保留现有 PPTX，同时提供最终版高质量设计大纲，方便按最新系统状态更新和演讲。 |
...
这个定位比“普通语音问答展示”更完整，因为它同时覆盖了课程 A2 的级联链路要求、低延迟指标要求、实验复现要求，以及信息安全课程中应当体现的安全边界与对抗输入防护。

## 四、答辩时最应该强调的三点

第一，系统是真实可运行的前后端应用。前端页面只是入口，后端有 FastAPI API、WebSocket 事件流、SQLite 持久化、管理后台、知识库 CRUD、运行审计、评测任务和导出能力。如果需要说明“是不是只是网页”，可以直接展示 `/api/health`、`/docs`、`/admin.html`、运行日志和测试结果。
...
第三，系统有可量化评测证据。当前安全门控评测 56 条样本，决策准确率 100%，false allow 为 0；在线 ASR 评测 50 条录音，平均 CER/WER 1.58%，术语召回 85.71%；多轮问答评测 6 组对话 18 轮，follow-up grounding accuracy 为 100%；引用质量评测 title hit@3 和 ID hit@3 均为 100%；真实语音链路在远程 GPU 环境完成 300 次 real-only 重复实验，gate-allowed 配对样本中流式改进平均节省 4147.39 ms，浏览器端 20/20 首播成功。
...
3. 本地运行 `python -m pytest tests -q` 通过。
...
9. 如需刷新 A2 补充证据，运行 `python scripts\build_a2_audio_eval_manifest.py` 和 `python scripts\evaluate_waiting_experience.py`，并确认对应结果文件已放入 evidence。

## 六、当前版本需要如实说明的边界

为了避免答辩时被追问“是否已经完全企业生产级”，建议在报告和答辩中主动说明边界。当前项目已经显著超过课程 A2 的基本要求，但仍是课程项目与比赛原型阶段，不应夸大为已经生产部署的工业系统。具体来说，真实端到端语音链路已经完成 300 次课程规模固定音频集重复实验，但还不是长期生产压测；浏览器首播平均约 4 秒，已经优于串行基线，但距离企业级自然接话体验还有优化空间；主观等待体验当前采用自动化代理评分，不是真人问卷；当前数据库是 SQLite，适合课程演示和单机原型，企业级部署应迁移到 PostgreSQL 并补充 RBAC、监控告警、审批流和灰度发布机制。
